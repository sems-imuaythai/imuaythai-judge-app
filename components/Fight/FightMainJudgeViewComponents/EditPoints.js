...
class EditPoints extends Component {
  ...
  render() {
    ...
    let mappedRedPointButtons = numberArray.map((val, i) => <PointButton key={ i } pointValue={ val } setPoint={ () => this.redSetPoint(val) } selected={ this.state.redPoints === val } color='red' />)
    let mappedBluePointButtons = numberArray.map((val, i) => <PointButton key={ i } pointValue={ val } setPoint={ () => this.blueSetPoint(val) } selected={ this.state.bluePoints === val } color='blue' />)
    return (
      <Grid style={ { backgroundColor: '#b2b2b2' } }>
        <Col>
        <Row style={ { alignContent: 'center', alignItems: 'center' } }>
          <Grid>
            { mappedRedPointButtons }
          </Grid>
        </Row>
        <Row>
          <Grid>
            { mappedBluePointButtons }
          </Grid>
        </Row>
